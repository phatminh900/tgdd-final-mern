...
// in order to set jwt
export const getToApi = (url: string, data: undefined, jwt?: string) => {
  return jwt ? axios.get(url, setAuthorizationHeader(jwt)) : axios.get(url);
};
export const postToApi = (url: string, data: any, jwt?: string) => {
  return jwt
    ? axios.post(`${url}`, data, setAuthorizationHeader(jwt))
    : axios.post(`${url}`, data);
};
export const patchToApi = (url: string, data: any, jwt?: string) => {
  return jwt
...
export const deleteToApi = (url: string, data: undefined, jwt?: string) => {
  return jwt
    ? axios.delete(url, setAuthorizationHeader(jwt))
    : axios.delete(url);
};
export const getTypeHttpMethod = (type: HTTPMethod) => {
  switch (type) {
    case "get":
      return getToApi;
    case "post":
